...
async function generateDatasourceTemplates(projectPath, projectManifest) {
    var _a;
    if (!projectManifest.isV0_2_1)
        return;
    const manifest = projectManifest.asV0_2_1;
    if (!((_a = manifest.templates) === null || _a === void 0 ? void 0 : _a.length))
        return;
    try {
        const props = manifest.templates.map((t) => ({
            name: t.name,
            args: (0, common_1.isCustomDs)(t) ? 'Record<string, unknown>' : undefined,
        }));
